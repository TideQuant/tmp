...
class PreprocessorNLogWMZ:
    ...
    def _init_window_mapping(self):
        """初始化时间窗口映射（从pattern_norm文件夹动态读取窗口）"""
        # 动态读取pattern_norm文件夹中的所有统计量文件
        self.windows = self._generate_time_windows_from_files()
    ...
    def _generate_time_windows_from_files(self):
        """从pattern_norm文件夹动态生成时间窗口列表（兼容任意窗口大小）"""
        import os
        import re
        ...
        # 检查pattern_norm目录是否存在
        if not os.path.exists(self.pattern_norm_dir):
            print(f"【预处理】警告：pattern_norm目录不存在: {self.pattern_norm_dir}")
            return windows
        
        # 读取所有统计量文件名
        files = sorted([f for f in os.listdir(self.pattern_norm_dir) if f.endswith('.h5')])
        
        # 解析文件名，提取窗口信息
        pattern = re.compile(r'temporal_norm_stats_(\d{8})_(\d{8})\.h5')
        window_id = 0
        
        for filename in files:
            match = pattern.match(filename)
            if match:
                start_str_raw = match.group(1)  # YYYYMMDD
                end_str_raw = match.group(2)    # YYYYMMDD
                
                # 转换为 YYYY-MM-DD 格式
                start_date = datetime.strptime(start_str_raw, "%Y%m%d")
                end_date = datetime.strptime(end_str_raw, "%Y%m%d")
        ...
        if len(windows) == 0:
            print(f"【预处理】警告：pattern_norm目录中没有找到统计量文件: {self.pattern_norm_dir}")
        else:
            print(f"【预处理】动态加载窗口映射: 共 {len(windows)} 个窗口")
            print(f"  第一个窗口: {windows[0][1]} 至 {windows[0][2]}")
            print(f"  最后一个窗口: {windows[-1][1]} 至 {windows[-1][2]}")
    ...
    def _load_stats(self, stats_file_path: str):
        ...
        with h5py.File(stats_file_path, "r") as f:
            self.stats = {
                'sta1': torch.from_numpy(f['sta1'][:]).float(),
                'sta2': torch.from_numpy(f['sta2'][:]).float(),
                'sta3': torch.from_numpy(f['sta3'][:]).float(),
                'sta4': torch.from_numpy(f['sta4'][:]).float(),
                'sta5': torch.from_numpy(f['sta5'][:]).float(),
            }
    ...
    def transform_batch(self, X: torch.Tensor, valid_mask: torch.Tensor, config: Dict, 
        ...
        参数:
            ...
            valid_mask: 掩码
            config: 配置字典
            datetime_array: 所有时间戳数组 (T,)，可选，用于动态加载统计量
            time_indices: 时间索引列表，可选，用于动态加载统计量
            show_progress: 是否显示进度
        """
        # 检查是否启用时序归一化
        # 修改逻辑：优先使用固定统计量（生产环境模式）
        ...
        use_dynamic_stats = enable_temporal_norm and self.stats is None and self.pattern_norm_dir is not None
        ...
        if X.dim() == 4:
            ...
            # 优化：预先加载batch所需的所有统计量
            batch_stats_map = {}
            if use_dynamic_stats and datetime_array is not None and time_indices is not None:
                batch_stats_map = self._preload_stats_for_batch(datetime_array, time_indices, seq, device)
            ...
            for b_idx in range(b):
                ...
                for s_idx in range(seq):
                    ...
                    if time_indices is not None:
                        # time_indices是batch级别的时间索引列表
                        if b_idx < len(time_indices):
                            # 对于seq_len=1的情况，time_indices[b_idx]就是当前时间索引
                            # 对于seq_len>1的情况，需要根据s_idx计算
                            if seq == 1:
                                time_idx = time_indices[b_idx]
                            else:
                                # seq_len>1时，time_indices[b_idx]是最后一个时间索引
                                # 当前切片的时间索引 = time_indices[b_idx] - (seq - 1 - s_idx)
                                time_idx = time_indices[b_idx] - (seq - 1 - s_idx)
                    
                    # 对每个切片应用预处理（传入预加载的统计量）
                    Y_t = self.transform_slice_vectorized(
                        ...
                        time_idx=time_idx,
                        preloaded_stats=batch_stats_map.get(time_idx) if time_idx is not None else None
                    )
                    seq_list.append(Y_t)
            ...
            for s_idx in range(seq):
                ...
                # 确定时间索引
                time_idx = None
                if time_indices is not None and s_idx < len(time_indices):
                    time_idx = time_indices[s_idx]
    ...
    def _apply_precomputed_temporal_norm(
        ...
        norm_method = temporal_norm_cfg.get("method", "zscore")
        
        N, F = X_t.shape
        
        if norm_method == "zscore":
            s1 = stats_device['sta1']
            s2 = stats_device['sta2']
            X_normalized = (X_t - s1) / (s2 + eps)
        
        elif norm_method == "minmax":
            s3 = stats_device['sta3']
            s4 = stats_device['sta4']
            ranges = s4 - s3
            valid = ranges > eps
            X_normalized_full = 2.0 * (X_t - s3) / (ranges + eps) - 1.0
            ...
            s5 = stats_device['sta5']
            s5_abs = torch.abs(s5)
            s5_abs = torch.where(s5_abs < eps, torch.ones_like(s5_abs), s5_abs)
            X_normalized = X_t / (s5_abs + eps)
        ...
        if invalid_mask.any():
            X_normalized = torch.where(invalid_mask, s5, X_normalized)
    ...
    def transform_slice_vectorized(
        ...
        if enable_temporal_norm:
            # 优先使用预加载的统计量（优化：避免重复加载）
            if preloaded_stats is not None:
                stats_to_use = preloaded_stats
            elif use_precomputed_stats and self.stats is not None:
                # 静态模式：使用预加载的统计量
                stats_to_use = self.stats
            elif self.pattern_norm_dir is not None and datetime_array is not None and time_idx is not None:
                # 动态模式：根据日期加载统计量（fallback，如果预加载失败）
                if time_idx >= 0 and time_idx < len(datetime_array):
                    timestamp = int(datetime_array[time_idx])
                    # 从时间戳提取日期（YYYYMMDD）
                    date_int = timestamp // 10000  # 时间戳格式：YYYYMMDDHHMM
                    stats_to_use = self._load_stats_for_date(date_int, device)
